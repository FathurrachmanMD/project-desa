...
interface Props {
  ...
  id?: number;
  penduduk?: FormData;
  // Inertia passes validation errors through props
...
const inputs: { [key: string]: InputConfig } = {
  ...
  status: { 
    ...
    type: 'select', 
    required: true, 
    options: ['aktif', 'nonaktif', 'suspended'] 
  }
};
...
export default function PendudukForm({ slug, id, penduduk }: Props) {
  const isEditMode = !!penduduk;
  const [data, setData] = useState<FormData>(penduduk || createDefaultData());
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // No need to setIsSubmitting here, router's onStart will handle it.
    ...
    if (isEditMode) {
        ...
        router.post(route('customers.store'), data, {
            ...commonOptions,
            onSuccess: () => {
                showToast.success("Penduduk baru berhasil ditambahkan!");
                setData(createDefaultData()); // Optionally reset form
            },
        });
    }
  };
        ...
        <div className="mb-8 bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="space-y-2">
            <h1 className="text-2xl font-bold text-gray-900 leading-tight">
              {isEditMode ? 'Edit Data Penduduk' : 'Formulir Penduduk Baru'}
            </h1>
